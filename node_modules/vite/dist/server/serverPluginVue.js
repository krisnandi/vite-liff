...
exports.vuePlugin = ({ root, app, resolver, watcher, config }) => {
    ...
    app.use(async (ctx, next) => {
        ...
        if (query.type === 'template') {
            ...
            ctx.type = 'js';
            ctx.body = compileSFCTemplate(root, templateBlock, filename, publicPath, descriptor.styles.some((s) => s.scoped), config.vueCompilerOptions);
            return etagCacheCheck(ctx);
        }
        if (query.type === 'style') {
            ...
            const result = await compileSFCStyle(root, styleBlock, index, filename, publicPath);
...
async function parseSFC(root, filename, content) {
    ...
    if (errors.length) {
        ...
        errors.forEach((e) => {
            console.error(chalk_1.default.underline(`${filename}:${e.loc.start.line}:${e.loc.start.column}`));
...
function compileSFCTemplate(root, template, filename, publicPath, scoped, userOptions) {
    let cached = exports.vueCache.get(filename);
    ...
    if (result.errors.length) {
        ...
        result.errors.forEach((e) => {
            if (typeof e === 'string') {
                ...
                if (e.line && e.column) {
                    console.log(chalk_1.default.underline(`${filename}:${e.line + lineOffset}:${e.column}`));
                }
                else {
                    console.log(chalk_1.default.underline(filename));
                }
                const filenameRE = new RegExp('.*' +
                    path_1.default.basename(filename).replace(/[-[\]/{}()*+?.\\^$|]/g, '\\$&') +
                    '(:\\d+:\\d+:\\s*)?');
