...
exports.hmrPlugin = ({ root, app, server, watcher, resolver, config }) => {
    ...
    function handleJSReload(filePath, timestamp = Date.now()) {
        ...
        if (importers) {
            ...
            const hasDeadEnd = walkImportChain(publicPath, importers, vueBoundaries, jsBoundaries, dirtyFiles);
...
function walkImportChain(importee, importers, vueBoundaries, jsBoundaries, dirtyFiles, currentChain = []) {
    ...
    for (const importer of importers) {
        if (importer.endsWith('.vue')) {
            ...
            if (!parentImpoters) {
                ...
                hasDeadEnd = walkImportChain(importer, parentImpoters, vueBoundaries, jsBoundaries, dirtyFiles, currentChain.concat(importer));
            }
        }
    }
    return hasDeadEnd;
...
function rewriteFileWithHMR(root, source, importer, resolver, s) {
    ...
    const checkHotCall = (node, isTopLevel, isDevBlock) => {
        if (node.type === 'CallExpression' &&
            ...
            if (isTopLevel) {
                console.warn(chalk_1.default.yellow(`[vite warn] HMR syntax error in ${importer}: hot.accept() should be` +
            ...
            if (node.callee.property.name === 'accept') {
                if (!isDevBlock) {
                    console.error(chalk_1.default.yellow(`[vite] HMR syntax error in ${importer}: hot.accept() cannot be ` +
    ...
    const checkStatements = (node, isTopLevel, isDevBlock) => {
        ...
        if (node.type === 'IfStatement') {
            const isDevBlock = node.test.type === 'Identifier' && node.test.name === '__DEV__';
