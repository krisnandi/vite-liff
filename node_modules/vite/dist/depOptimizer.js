...
const buildPluginCss_1 = require("./build/buildPluginCss");
...
async function optimizeDeps(config, asCommand = false) {
    ...
    const resolver = resolver_1.createResolver(root, config.resolvers, config.alias);
    ...
    if (!qualifiedDeps.length) {
        if (!cjsDeps.length) {
            ...
            console.error(chalk_1.default.yellow(`[vite] The following dependencies seem to be CommonJS modules that\n` +
                `do not provide ESM-friendly file formats:\n\n  ` +
                cjsDeps.map((dep) => chalk_1.default.magenta(dep)).join(`\n  `) +
                `\n` +
                `\n- If you are not using them in browser code, you can move them\n` +
                `to devDependencies or exclude them from this check by adding\n` +
                `them to ${chalk_1.default.cyan(`optimizeDeps.exclude`)} in vue.config.js.\n` +
                `\n- If you do intend to use them in the browser, you can try adding\n` +
                `them to ${chalk_1.default.cyan(`optimizeDeps.commonJSWhitelist`)} in vue.config.js but they\n` +
    ...
    try {
        ...
        const bundle = await rollup.rollup({
            ...
            plugins: [
                ...
                buildPluginCss_1.createBuildCssPlugin(root, '/', 'assets')
            ]
        });
        ...
        if (asCommand) {
            ...
            console.error(chalk_1.default.red(`[vite] Dep optimization failed with error:`));
            console.error(e);
            console.log();
            console.log(chalk_1.default.yellow(`Tip: You can configure what deps to include/exclude for optimization\n` +
